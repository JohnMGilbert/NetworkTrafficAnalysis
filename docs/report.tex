...
The clustering experiments reused the stratified 100,000-row UMAP sample and fit final clustering models on a stratified 5,000-row subset, with hyperparameter selection on a 2,000-row subset. This sampling choice follows the assignment's allowance for stratified random samples on computationally expensive clustering. The three required clustering families were MiniBatchKMeans, HDBSCAN, and GaussianMixture. Table~\ref{tab:q2_cluster_config} lists the selected configurations and cluster counts, and Figure~\ref{fig:q2_clusters} shows the embeddings.
...
HDBSCAN & Density-based & 5 plus noise & \texttt{min\_cluster\_size=250}, \texttt{min\_samples=10}, \texttt{cluster\_selection\_method=eom}; 12.72\% noise \\
GaussianMixture & Model-based & 14 & \texttt{n\_components=14}, diagonal covariance, \texttt{reg\_covar=0.0001}, \texttt{max\_iter=200}, \texttt{init\_params=kmeans} \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{figure}[!t]
\centering
\includegraphics[width=0.96\linewidth]{q2_2a_cluster_embedding_comparison.png}
\caption{Q2.2(a) MiniBatchKMeans, HDBSCAN, and GaussianMixture cluster assignments projected onto the Q2.1(b) UMAP space.}
...
\midrule
HDBSCAN & 5 & 636 & 12.72 & 0.4719 & 0.9041 & 3387.73 \\
GaussianMixture & 14 & 0 & 0.00 & 0.4311 & 1.3676 & 1364.52 \\
MiniBatchKMeans & 8 & 0 & 0.00 & 0.3715 & 1.3678 & 834.01 \\
\bottomrule
\end{tabular}
\end{table}

For Q2.2(c), HDBSCAN was selected for semantic mapping because it had the strongest internal validation profile and handles noise and imbalanced density better than methods that force every sample into a cluster. Table~\ref{tab:q2_mapping} gives the confusion-style mapping to the known classes. Cluster 0 maps to DDoS TCP because flows are tiny and one-directional; Cluster 1 maps to DDoS Bot because it contains large asymmetric flows concentrated on service ports; Cluster 2 maps to Normal because it contains short balanced exchanges with more port diversity; Cluster 3 maps to DoS Hulk because it has sustained request-burst behavior; Cluster 4 maps to DoS SlowHTTP because it contains long-lived low-rate flows. Noise remains Unknown rather than being over-labeled.
...
\midrule
-1 & Unknown & 636 & 12.72\% & Low \\
0 & DDoS TCP & 429 & 8.58\% & High \\
1 & DDoS Bot & 1406 & 28.12\% & Medium \\
2 & Normal & 1101 & 22.02\% & Medium \\
3 & DoS Hulk & 1040 & 20.80\% & Medium \\
4 & DoS SlowHTTP & 388 & 7.76\% & High \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
DDoS TCP & 1 & 9.672 & Minimal packets and one-way asymmetry \\
DoS SlowHTTP & 2 & 9.155 & Long-lived, very low-rate connections \\
DDoS Bot & 3 & 7.562 & Large asymmetric bot-driven floods \\
DoS Hulk & 4 & 7.416 & Burst-heavy HTTP flooding \\
Normal & 5 & 5.640 & Broad but still identifiable balanced traffic \\
...
\centering
Input flow $\rightarrow$ 79 original CICFlowMeter features $\rightarrow$ tuned RandomForest backbone $\rightarrow$ per-class score calibration $\rightarrow$ selected advanced prediction
\end{minipage}}
