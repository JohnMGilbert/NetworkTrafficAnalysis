...
Embedding sample & 100,000 rows, stratified to preserve the observed router distribution across 10 routers \\
...
The selected configurations were MiniBatchKMeans with \texttt{n\_clusters=8}, \texttt{batch\_size=4096}, \texttt{max\_iter=200}, and \texttt{n\_init=10}; HDBSCAN with \texttt{min\_cluster\_size=250}, \texttt{min\_samples=10}, and \texttt{cluster\_selection\_method=eom}; and GaussianMixture with \texttt{n\_components=14}, \texttt{covariance\_type=diag}, \texttt{reg\_covar=0.0001}, \texttt{max\_iter=200}, and \texttt{init\_params=kmeans}.

Table~\ref{tab:internal_validation} shows the final internal validation results from Question 2.2(b). HDBSCAN produced the strongest overall profile, with the best silhouette score, the lowest Davies-Bouldin Index, and the highest Calinski-Harabasz Index. It recovered five non-noise clusters and labeled 12.72\% of the fitted sample as noise. That behavior is well matched to this assignment because the data are highly imbalanced; HDBSCAN can preserve dense majority structures without forcing rare or ambiguous flows into an artificial cluster.
...
\midrule
HDBSCAN & 5 & 12.72 & non-noise only & 0.4719 & 0.9041 & 3387.73 \\
GaussianMixture & 14 & 0.00 & full sample & 0.4311 & 1.3676 & 1364.52 \\
MiniBatchKMeans & 8 & 0.00 & full sample & 0.3715 & 1.3678 & 834.01 \\
\bottomrule
\end{tabularx}
\end{table}
...
\midrule
$-1$ & Unknown & low & 636 & 12.72 \\
0 & DDoS TCP & high & 429 & 8.58 \\
1 & DDoS Bot & medium & 1,406 & 28.12 \\
2 & Normal & medium & 1,101 & 22.02 \\
3 & DoS Hulk & medium & 1,040 & 20.80 \\
4 & DoS SlowHTTP & high & 388 & 7.76 \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{itemize}
\item \textbf{Cluster 0 to DDoS TCP (high confidence):} This cluster has extremely small flows, with mean 1.97 packets and 112.01 bytes per flow, and it is almost completely one directional with a mean down/up ratio of 0.00. That is consistent with compact TCP flood behavior that generates many low-content requests with minimal response traffic.
\item \textbf{Cluster 1 to DDoS Bot (medium confidence):} This group contains large flows, averaging 108.93 packets and 81,243.86 bytes, with strong forward/backward packet-size asymmetry (0.84). Its traffic is concentrated on a narrow set of service ports, especially 7777 and 8080, which supports the interpretation of coordinated bot-driven flooding.
\item \textbf{Cluster 2 to Normal (medium confidence):} This cluster consists of short, balanced exchanges with mean duration 1,152, mean 3.67 packets, and a down/up ratio of 1.00. Its destination ports are more varied than the attack-oriented clusters, making it the strongest candidate for routine benign background traffic in the fitted sample.
\item \textbf{Cluster 3 to DoS Hulk (medium confidence):} This cluster has sustained flows with mean duration 13,728,396 and about 105.92 packets per flow. The behavior is more burst-like than benign traffic and is concentrated on a small set of destination ports, which is consistent with a Hulk-style HTTP flooding pattern.
\item \textbf{Cluster 4 to DoS SlowHTTP (high confidence):} These flows are very long lived, with mean duration 30,921,802, but extremely low rate at only 3.38 packets per second. That long-duration, low-rate profile is a strong fit for SlowHTTP-style application-layer connection exhaustion.
\item \textbf{Noise cluster to Unknown (low confidence):} HDBSCAN treated 636 flows as noise or outliers. Their destination ports are more mixed than the dense clusters and their average byte volume is comparatively high, which suggests a blend of rare attacks, ambiguous flows, and low-density behavior that should not be over-labeled without stronger evidence.
\end{itemize}
